...
class TestHasUncheckedTasks:
    ...
    def test_hierarchical_tasks_with_checked_parent(self, tmp_path: Path) -> None:
        """Phase with checked parent and unchecked sub-tasks should be considered complete.

        This tests the fix for the bug where sub-tasks under a completed parent
        were incorrectly preventing phase transitions.
        """
        plan_content = """# Implementation Plan

## Phase 4: TEST

- [x] Write 70+ integration tests in test_branch_sync_service.py:
  - [ ] Test sync_all_branches creates new Branch records
  - [ ] Test sync_all_branches updates existing branches
  - [ ] Test detecting merged branches
  - [ ] Test detecting deleted branches
  - [ ] Test idempotency
  - [ ] Test GitHub API errors

- [x] Write 10+ Playwright tests in e2e/branches.spec.ts:
  - [ ] Test branch list displays with filters
  - [ ] Test branch list search functionality

- [x] Run all tests: just test
- [x] Fix any test failures
"""
        plan_file = tmp_path / "plan.md"
        plan_file.write_text(plan_content)

        # Phase 4 should be considered complete because all top-level tasks are checked
        # Even though sub-tasks are unchecked, they're under completed parents
        assert not has_unchecked_tasks(Phase.TEST, plan_file)

    def test_hierarchical_tasks_with_unchecked_parent(self, tmp_path: Path) -> None:
...
"""
    ...
    def test_test_advances_when_complete(self, all_complete_plan_file: Path) -> None:
        """Phase 3 advances to Phase 4 when all tests pass."""
        next_phase = determine_next_phase(Phase.TEST, all_complete_plan_file)
        assert next_phase == Phase.REVIEW

    def test_review_returns_to_implement_when_fixes_needed(self, tmp_path: Path) -> None:
        """Phase 4 returns to Phase 2 when it adds Fix tasks to Phase 2 (critical issues found)."""
        plan_content = """
## Phase 2: IMPLEMENT
...
"""
    ...
    def test_review_advances_when_complete(self, all_complete_plan_file: Path) -> None:
        """Phase 4 advances to Phase 5 when all tasks checked."""
    ...
    def test_commit_returns_none(self, sample_plan_file: Path) -> None:
        """Phase 5 returns None (workflow complete)."""
...
"""
    ...
    def test_final_review_returns_to_implement(self, tmp_path: Path) -> None:
        ...
        plan_content = """
## Phase 2: IMPLEMENT
- [ ] Fix: Critical issue found during review
...
class TestComprehensiveMode:
    ...
    def test_complete_comprehensive_workflow(self, all_complete_plan_file: Path) -> None:
        """Test complete 7-phase workflow in comprehensive mode."""
        phases = [
        ...
        for current, expected_next in phases:
            next_phase = determine_next_phase(current, all_complete_plan_file, comprehensive=True)
            assert next_phase == expected_next

    def test_middle_phases_same_in_both_modes(self, all_complete_plan_file: Path) -> None:
        """Middle phases (IMPLEMENT through REVIEW) work same in both modes."""
        middle_phases = [
            (Phase.IMPLEMENT, Phase.TEST),
            (Phase.TEST, Phase.REVIEW),
            (Phase.REVIEW, Phase.COMMIT),
